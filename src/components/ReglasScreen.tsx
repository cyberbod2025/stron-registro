    ...
    >
      ...
      <div className="text-center mb-10">
        <motion.div
          ...
          className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br from-[#ff4994] to-[#582ea2] mb-4 shadow-[0_0_30px_rgba(255,73,148,0.3)]"
        >
          <span className="text-2xl">💪</span>
        </motion.div>
        <h1 className="text-2xl font-black italic text-white uppercase tracking-tight">
          Compromiso{" "}
          <span className="text-[#ff4994]">STRONG</span>
          {" "}💪
        </h1>
      </div>
      ...
      <div className="space-y-4 mb-10">
        {rules.map((rule, index) => (
          <motion.div
            ...
            className="flex items-start gap-4 p-4 rounded-2xl bg-[#2a1520]/40 border border-white/5"
          >
            <div className="w-10 h-10 rounded-xl bg-[#ff4994]/10 border border-[#ff4994]/20 flex items-center justify-center shrink-0 mt-0.5">
              <span className="material-symbols-outlined text-[#ff4994] text-xl">{rule.icon}</span>
            </div>
            <p className="text-sm text-[#e2bdc6] leading-relaxed pt-2">
              {rule.text}{" "}
              <strong className="text-[#ff4994] font-extrabold">{rule.highlight}</strong>
              {rule.suffix}
            </p>
          </motion.div>
        ))}
      ...
      <motion.div
        ...
        className="text-center p-6 rounded-2xl bg-gradient-to-br from-[#ff4994]/10 to-purple-900/10 border border-[#ff4994]/20"
      >
